...
def main(instance_file, str_time_limit, sol_file):
    ...
    # Reads instance data
    #
    (nb_customers, nb_trucks, truck_capacity, distance_matrix, distance_warehouses, demands,
        service_time, earliest_start, latest_end, max_horizon) = read_input_cvrptw(instance_file)
    ...
    with localsolver.LocalSolver() as ls:
        ...
        for k in range(nb_trucks):
            ...
            home_lateness[k] = model.iif(trucks_used[k], \
               model.max(0, end_time[k][c - 1] + model.at(distance_warehouse_array, sequence[c - 1]) - max_horizon), \
                0)
