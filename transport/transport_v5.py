...
with localsolver.LocalSolver() as ls:
    ...
    x_max = 1000
    try:
        x = [[[[model.int(0, x_max) for i in range(len(plants))] for j in range(len(markets))] for t in range(len(periods))] for p in
             range(len(products))]
        ...
        try:
            for i in range(0, len(plants)):
                for p in range(0, len(products)):
                    model.constraint(
                        model.sum(x[p][t][j][i] for j in range(len(markets)) for t in range(len(periods))) <= capacity[(plants[i], products[p])])
        except Exception as exception_msg:
